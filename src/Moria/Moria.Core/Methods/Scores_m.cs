...
namespace Moria.Core.Methods
{
    public static class Scores_m
    {
        ...
        public static void recordNewHighScore()
        {
            ...
            HighScore_t new_entry = new HighScore_t();
            ...
            new_entry.dungeon_depth = (uint)dg.current_level;
            new_entry.level = (uint)py.misc.level;
            new_entry.deepest_dungeon_depth = (uint)py.misc.max_dungeon_depth;
            new_entry.gender = highScoreGenderLabel();
            new_entry.race = py.misc.race_id;
            new_entry.character_class = py.misc.class_id;
            new_entry.name = py.misc.name;
            //(void)strcpy(new_entry.name, py.misc.name);

            var tmp = game.character_died_from;
            if (tmp.StartsWith("an "))
            {
                tmp = tmp.Substring(3);
            ...
            var version_maj = (uint8_t)getc(highscore_fp);
            var version_min = (uint8_t)getc(highscore_fp);
            var patch_level = (uint8_t)getc(highscore_fp);
        ...
        public static int playerCalculateTotalPoints()
        {
            var py = State.Instance.py;
            var dg = State.Instance.dg;
